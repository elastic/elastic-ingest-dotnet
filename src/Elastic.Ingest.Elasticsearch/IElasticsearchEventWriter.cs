...
public interface IElasticsearchEventWriter<TEvent>
{
	...
	/// <para>This implementation is only called if <see cref="IngestChannelOptionsBase{TEvent}.UseReadOnlyMemory"/> is true, defaults to false</para>
	/// <para>Otherwise <see cref="WriteToStreamAsync"/> is called instead.</para>
	/// <para>If `null` <see cref="IngestChannelBase{TEvent,TChannelOptions}"/> will fallback to its own internal implementation.</para>
	/// </summary>
	...
	/// <para>If `null` <see cref="IngestChannelBase{TEvent,TChannelOptions}"/> will fallback to its internal implementation.</para>
	/// </summary>
